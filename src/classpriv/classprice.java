...
public class classprice extends javax.swing.JFrame {
 Connection con ;
    ...
    private void initComponents() {
        ...
        cmclass.setModel(new javax.swing.DefaultComboBoxModel(new String[] { " " }));
    ...
    private void saveActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_saveActionPerformed
        if( id.getText().trim().isEmpty() && !price.getText().trim().isEmpty() )
                 ...
                 try
           {
               int amountt = Integer.parseInt(price.getText());
                 String sql111 = "INSERT INTO mrahmed.lessonprice "
                         + "( lessonpricee, classname) VALUES ( '"+amountt+"', '"+cmclass.getSelectedItem().toString()+"')";
                        pst = (PreparedStatement)con.prepareStatement(sql111);
                        pst.execute();
                        JOptionPane.showMessageDialog(null, "تم ـم تسجيل سعر الحصه بنجاح");
        ...
        if(!id.getText().trim().isEmpty() && !price.getText().trim().isEmpty())
        {
            try {
                ...
                int amountt = Integer.parseInt(price.getText());
                String sqll = "UPDATE mrahmed.lessonprice SET lessonpricee = '"+amountt+"' WHERE idlessonprice = '"+id.getText()+"'";
                pst=(PreparedStatement) con.prepareStatement(sqll);
                pst.executeUpdate();
                JOptionPane.showMessageDialog(rootPane, "تم تعديل بيانات سعر الحصه بنجاح");
                price.setText("");
                cmclass.setSelectedIndex(0);
            } catch (SQLException ex) {
                Logger.getLogger(classprice.class.getName()).log(Level.SEVERE, null, ex);
            }
...
public void classes()
   {
        try {
             String sql="SELECT classname  FROM mrahmed.class where classname='الثالت الثانوي'  ";
          ...
          public void comboxfromcombo1()
      {
            ...
            public void itemStateChanged(ItemEvent event) {
             ...
             String sql="SELECT lessonpricee  FROM mrahmed.lessonprice "
                     + "where classname='"+cmclass.getSelectedItem().toString()+"'";
             pst=(PreparedStatement) con.prepareStatement(sql);
             rs=pst.executeQuery();
             while(rs.next()){
                 price.setText(rs.getString("lessonpricee"));
                ...
                if (event.getStateChange() == ItemEvent.DESELECTED) {
                     ...
                     try{
                         ...
                         {
                             String sql="SELECT idlessonprice  FROM mrahmed.lessonprice where classname='"+cmclass.getSelectedItem().toString()+"'";
             pst=(PreparedStatement) con.prepareStatement(sql);
             rs=pst.executeQuery();
             while(rs.next()){
                 id.setText(rs.getString("idlessonprice"));
                 price.setText("");
             }
                         }
                     }
